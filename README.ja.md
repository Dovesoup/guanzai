...
GuanZai は、ローカル AI エージェント向けの、小さく決定論的なガバナンスおよび計画レイヤーです。仕組みを増やす前に、三つの問いを立てます。この作業は solo のままにすべきか、別のエージェントを加えることはコストに見合うか、そしてその結果には独立監査が必要か。

技術システムと人間の判断が交わる場所で作られています。目指すのは最大規模のチームを呼び集めることではなく、人が制御を保てる余白を十分に残しながら、意図を持って選択することです。
