...
STRONG_READ_ONLY_ACTION = re.compile(r"只分析|只读|仅查看")
STRONG_NOMINAL_IMPLEMENTATION = re.compile(r"实现(?:原理|方式|细节|机制|思路|方案)")
NOMINAL_IMPLEMENTATION = re.compile(r"(?<![和与并或、及])实现(?:原理|方式|细节|机制|思路|方案)")
CLEAR_NOMINAL_MUTATION = re.compile(
    r"(?:修改|修复|更新)(?:建议(?!内容)|说明(?!文档)|记录(?!页面|的)|历史(?!记录(?:页面|的)))"
)
AMBIGUOUS_NOMINAL_MUTATION = re.compile(r"(?<![和与并或、及])(?:修改|修复|更新)方案")
...
BUILD_ACTION = re.compile(r"开发|实现|编程|迁移|修复|(?:修改|新增|更新)[^，,。；;！？!?]*(?:接口|系统|配置|设置|测试|仪表盘|代码|前端|后端|编译器|方案|文档|页面)")
...
def _has_mutation(task: str) -> bool:
    for scope in MAIN_SCOPE_BOUNDARY.split(task):
        ...
        if READ_ONLY_ACTION.search(scope):
            scope = CLEAR_NOMINAL_MUTATION.sub("", scope)
            scope = AMBIGUOUS_NOMINAL_MUTATION.sub("", scope)
            nominal_implementation = (
                STRONG_NOMINAL_IMPLEMENTATION if STRONG_READ_ONLY_ACTION.search(scope) else NOMINAL_IMPLEMENTATION
            )
