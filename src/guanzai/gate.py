...
NEGATED_MUTATION = re.compile(r"(?:不(?:需要|要|用)?|无需|请勿)(?:做|进行|执行)?(?:任何)?(?:修改|实现|开发|修复|创建|新增|更新|执行|写入|删除|发布|上线|部署|迁移|付款|转账|计费|账单|公式|授权)")
NOMINAL_IMPLEMENTATION = re.compile(r"实现(?:原理|方式|细节|机制|思路|方案)")
READ_ONLY_ACTION = re.compile(r"只读|仅查看|分析|解释|评估|审查|查看|阅读|理解|输出报告")
CLAUSE_BOUNDARY = re.compile(r"[、，,。；;！？!?]|然后|随后|并且|但是|但|再|和|并")
...
RESEARCH_ACTION = re.compile(r"调研|搜索|查找|检索|(?:整理|比较|收集)[^，,。；;！？!?]*(?:资料|竞品|证据|来源|数据|文献|信息)")
DESIGN = re.compile(r"设计|架构|方案|需求")
BUILD = re.compile(r"开发|实现|编程|代码|前端|后端|接口|系统|架构|迁移|编译器|仪表盘|(?:修改|新增)[^，,。；;！？!?]*(?:配置|设置)")
