...
GuanZai 是一个面向本地 AI 智能体的小型、确定性治理与规划层。在增加更多机制之前，它会先问三个问题：这项工作是否应该由编排者独自完成，增加另一个智能体是否值得其成本，以及其后果是否需要独立审计？

它构建于技术系统与人类判断交汇之处。目标不是召集尽可能庞大的团队，而是审慎选择，并为人保留足够的控制空间。

> [!WARNING]
> **公开 Alpha — `v0.3.0-alpha.1`。** GuanZai 生成计划和适配器命令；它还不是一个完整、无人值守的执行闭环。使用前请审查每一份清单和命令。接口与策略细节可能会变化。
