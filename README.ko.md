...
GuanZai는 로컬 AI 에이전트를 위한 작고 결정론적인 거버넌스 및 계획 계층입니다. 더 많은 장치를 추가하기 전에 세 가지를 묻습니다. 이 작업을 solo로 유지해야 하는가, 다른 에이전트를 추가할 가치가 비용보다 큰가, 그리고 그 결과에 독립 감사가 필요한가?

기술 시스템과 인간의 판단이 만나는 지점에서 만들어졌습니다. 목표는 가능한 한 큰 팀을 불러 모으는 것이 아니라, 사람이 통제권을 유지할 충분한 여백을 두고 신중하게 선택하는 것입니다.
